...
export function getPlaceholder(userOptions: Partial<PlaceholderOptions>): Placeholder {
  ...
  const placeholderClassList = ['wcbrd-placeholder-img', className].join(' ')
...
function getOptionsWithDefaults(options: Partial<PlaceholderOptions>) {
  const optionsWithDefaults = Object.assign(
    {},
    {
      // WCBR gray scale has 5 stops (100/200/300/400/700), not Bootstrap's 9 —
      // pick a mid gray for the background and a dark one for readable placeholder text.
      background: getData('grays')[3].hex,
      color: getData('grays')[4].hex,
      height: '180',
      markup: 'svg',
      title: 'Placeholder',
      width: '100%'
...
export interface PlaceholderOptions {
  ...
  background: string
  /**
   * CSS classes to append to `wcbrd-placeholder-img` for the `svg` or `img` elements.
   */
  class?: string
  /**
