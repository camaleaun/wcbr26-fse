...
// Copy assets (CSS/JS) to make them available under the versioned docs path.
function copyBootstrap() {
  const source = path.join(process.cwd(), 'assets')
  if (!fs.existsSync(source)) return

  const destination = path.join(getDocsPublicFsPath(), 'docs', getConfig().docs_version, 'assets')
